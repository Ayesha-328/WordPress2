...
class Functions{
    ...
    public function get_compare($post_id, $params = []) {
        ...
        $compare_icon = file_get_contents( WOPB_PATH . 'assets/img/svg/' . $this->get_setting('compare_button_icon') . '.svg' );
    ...
    ?>
        <a
            ...
            data-modal_wrapper_class="<?php echo $modal_wrapper_class; ?>"
        >
        <?php
            if( isset($params['source']) && $params['source'] == 'default' ) {
                echo $button_icon_enable == 'yes' && $compare_icon_position == 'before_text' ? $compare_icon : '';
        ...
        <?php
                ...
                echo $button_icon_enable == 'yes' && $compare_icon_position == 'after_text' ? $compare_icon : '';
            }else {
    ...
    public function get_wishlist_html($post_id, $wishlist_active, $layout , $position_left) {
        ...
        $action_added = $this->get_setting('wishlist_action_added');
            ...
            $html .= '<a class="wopb-wishlist-icon wopb-wishlist-add'.($wishlist_active ? ' wopb-wishlist-active' : '').'" data-action="add" '.($wishlist_page && $action_added == 'redirect' ? 'data-redirect="'.esc_url( get_permalink( $wishlist_page ) ).'"' : '').' data-postid="'.esc_attr( $post_id ).'" data-modal_content_class="wopb-wishlist-wrapper" data-modal-loader="loader_1">';
    ...
    public function get_quick_view($params = []) {
        if ( $this->get_setting('quickview_mobile_disable') == 'yes' && wp_is_mobile() ) {} else {
            ...
            $quick_view_icon = file_get_contents(WOPB_PATH . 'assets/img/svg/' . $this->get_setting('quick_view_button_icon') . '.svg');
            $quick_view_icon_position = $this->get_setting('quick_view_button_icon_position');
            $button_icon_enable = $this->get_setting('quick_view_button_icon_enable');
        ...
        ?>
            ...
            >
                <?php
                    if( isset($params['source']) && $params['source'] == 'default' ) {
                        echo $before_text . $quick_view_text . $after_text;
                    }elseif ($params['tooltip']) {
                ?>
                    <span class="wopb-tooltip-text">
                        <?php
                            echo $quick_view_icon;
                            if ($quick_view_text) {
                        ?>
    ...
    public function get_wishlist_id() {
        ...
        if ($required_login == 'yes' && $user_id) {
            ...
            $data = json_decode( wp_unslash( $_COOKIE['wopb_wishlist'] ) );
            if( is_array( $data ) ) {
                return $data;
            }else {
    ...
    public function get_quick_query($prams, $args, $sort_query = null) {
        switch ($sort_query) {
            case 'sales_day_1':
                $args['post__in'] = $this->get_best_selling_products( date('Y-m-d H:i:s', strtotime("-1 days") ) );
                break;
            case 'sales_day_7':
                $args['post__in'] = $this->get_best_selling_products( date('Y-m-d H:i:s', strtotime("-7 days") ) );
                break;
            case 'sales_day_30':
                $args['post__in'] = $this->get_best_selling_products( date('Y-m-d H:i:s', strtotime("-30 days") ) );
                break;
            case 'sales_day_90':
                $args['post__in'] = $this->get_best_selling_products( date('Y-m-d H:i:s', strtotime("-90 days") ) );
                break;
            case 'sales_day_all':
                $args['meta_key'] = 'total_sales';
                $args['orderby'] = 'meta_value_num';
	 ...
	 */
    public function svg_icon($icon_name = ''){
        if ( $icon_name ) {
            if ( file_exists( WOPB_PATH . 'assets/img/svg/' . $icon_name . '.svg' ) ) {
                return file_get_contents( WOPB_PATH . 'assets/img/svg/' . $icon_name . '.svg' );
            }
            return '';
        }
        return '';
...
?>
        <div class="wopb-modal-loading">
            <div class="wopb-loading">
                <span class="wopb-loader <?php echo $loader ?>">
                    <?php
