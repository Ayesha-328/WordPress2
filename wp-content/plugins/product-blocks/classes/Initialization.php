                ...
                <div class="wopb-loading">
                    <?php
                        ...
                        foreach ( $modal_loaders as $key => $loader ) {
                            echo '<span class="wopb-loader wopb-d-none ' . $key . '">';
...
<?php
    ...
    public function install_hook() {
        ...
        if ( ! empty( $data ) ) {
            ...
            if(!get_option('wopb_activation')) {
                update_option('wopb_activation',date('U'));
            }
    ...
    public function plugin_activation( $plugin ) {
        ...
        if ( $plugin == 'product-blocks/product-blocks.php' ) {
            $settings = wopb_function()->get_setting();
            if ( ! isset( $settings['css_save_as'] ) ) {
                exit( wp_safe_redirect( admin_url( 'admin.php?page=wopb-initial-setup-wizard' ) ) ); //phpcs:ignore
            } else {
                exit( wp_safe_redirect( admin_url( 'admin.php?page=wopb-settings' ) ) ); //phpcs:ignore
            }
        }
    }
